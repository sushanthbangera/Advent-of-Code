...
public class TreeCounter {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(getTotalTrees(input1, 3, 1));
        ...
        System.out.println(getTotalTrees(input2, 3, 1));
    }

    public static int getTotalTrees(List<String> inputMap, int right, int down) {
        int treeCount = 0;
        int currentPosition = 0;
        int rows = inputMap.size();
        int rowLen = inputMap.get(0).length();
        int row = down;
        while (row < rows) {
            currentPosition = getNextPosition(currentPosition, right);
            currentPosition = currentPosition % rowLen;
            char pos = inputMap.get(row).charAt(currentPosition);
            if (pos == TREE) {
                treeCount++;
            }
            row += down;
    ...
    private static int getNextPosition(int currentPosition, int right) {
