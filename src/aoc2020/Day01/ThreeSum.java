...
public class ThreeSum {
    ...
    public static void main(String[] args) {
        ...
        System.out.print(getThreeNumProd(arr));
    }

    public static long getThreeNumProd(int arr[]) {
        for (int i = 0; i < arr.length - 2; i++) {
            int[] subArr = IntStream.range(i + 1, arr.length)
                    .map(k -> arr[k])
                    .toArray();
            int twoProd = TwoSum.twoSum(subArr, 2020 - arr[i]);

            if (twoProd != -1) {
                return twoProd * arr[i];
            }
        }
        return -1;
    }
